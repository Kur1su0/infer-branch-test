...
int main(int argc, char **argv)
{
    ...
    if (UnitNumber == 7)
    { //TEST 7: addition and deletion test.   
      const int ins[] = {200, 100, 150};
      const int del[] = {52};
        unitDriver(ins, sizeof ins/sizeof(int), del, sizeof del/sizeof(int));
    }
    if (UnitNumber == 8)
    {//TEST 8.  remove node with no childern.
  ...
  const int del[] = {3, 5, 7};
    ...
    if (UnitNumber == 9)
    {//Test 9: remove head.
        const int ins[] = {10, 2, 5, 7,9};
        const int del[] = {10};
        unitDriver(ins, sizeof ins/sizeof(int), del, sizeof del/sizeof(int));
    }
    if (UnitNumber == 10)
    {//Test 10:  right of parent  //75 - right
      const int ins[] = {200, 100, 50, 150, 25, 75, 65, 85, 80, 78};
      const int del[] = {100};
        unitDriver(ins, sizeof ins/sizeof(int), del, sizeof del/sizeof(int));
    }
    
    if (UnitNumber == 11)
    {//Test 11:  node's left subtree max value's parent is the node to be deleted.
      const int ins[] = {200, 100, 50, 150, 25, 75, 65, 85, 80, 78};
      const int del[] = {75};
        unitDriver(ins, sizeof ins/sizeof(int), del, sizeof del/sizeof(int));
    }
    ...
    if (UnitNumber == 12)
    { //TEST 12: Poor tree Test   
      const int ins[] = {100, 200, 300, 400, 450, 500, 550, 600, 650, 700, 750,800,
                         850, 900, 950};
      const int del[] = {200, 100};
        unitDriver(ins, sizeof ins/sizeof(int), del, sizeof del/sizeof(int));
    }
...
void getCommandLine(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "w:t:s:f:v:e:o:r:u:p:q")) != -1)
        switch(c) {
